...
LaserScanMatcherOdometry::LaserScanMatcherOdometry(ros::NodeHandle nh, ros::NodeHandle nh_private) :
        nh_(nh),
...
{
    ROS_INFO("Starting LaserScanMatcherOdometry");
...
LaserScanMatcherOdometry::~LaserScanMatcherOdometry() {
...
void LaserScanMatcherOdometry::cloudCallback(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &cloud) {
...
void LaserScanMatcherOdometry::scanCallback(const sensor_msgs::LaserScan::ConstPtr &scan_msg) {
    // **** if first scan, cache the tf from base to the scanner
...
void LaserScanMatcherOdometry::imuCallback(const sensor_msgs::ImuPtr &imu_msg) {
...
void LaserScanMatcherOdometry::odomCallback(const nav_msgs::Odometry::ConstPtr &odom_msg) {
...
void LaserScanMatcherOdometry::processScan(LDP &curr_ldp_scan, const ros::Time &time) {
...
printf("%f, %f, %f\n", input_.first_guess[0],
    ...
    if (output_.valid)
    {
        ...
        if(use_alpha_beta_ || publish_lsm_odom)
        {
            ...
            double da = getYawFromQuaternion(w2b_new.getRotation()) - getYawFromQuaternion(w2b_.getRotation());
        ...
        if (publish_lsm_odom)
        {
            ...
            odom.pose.pose.position.x = w2b_.getOrigin().getX();
...
void LaserScanMatcherOdometry::PointCloudToLDP(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &cloud, LDP &ldp) {
    ...
    for (unsigned int i = 0; i < n; i++)
    {
        ...
        {
            ROS_WARN("Laser Scan Matcher: Cloud input contains NaN values. Please use a filtered cloud input.");
        }
        else
        {
...
void LaserScanMatcherOdometry::laserScanToLDP(const sensor_msgs::LaserScan::ConstPtr &scan_msg, LDP &ldp) {
...
void LaserScanMatcherOdometry::createCache(const sensor_msgs::LaserScan::ConstPtr &scan_msg) {
...
bool LaserScanMatcherOdometry::getBaseToLaserTf(const std::string &frame_id) {
...
void LaserScanMatcherOdometry::getPrediction(double &pr_ch_x, double &pr_ch_y, double &pr_ch_a, double dt) {
...
double LaserScanMatcherOdometry::getYawFromQuaternion(const tf::Quaternion &quaternion) {
    double temp, yaw;
...
double LaserScanMatcherOdometry::getYawFromQuaternion(const geometry_msgs::Quaternion& quaternion) {
...
void LaserScanMatcherOdometry::createTfFromXYTheta(double x, double y, double theta, tf::Transform &t) {
